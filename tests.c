...
void log_variable(const char *var_name, int value);
void update_data();
...
//    sensitivity_x = 100;
 //   sensitivity_y = 100;
    ...
    if (!mouse_reset())
    {
        log_message("Error: mouse not available. exiting.\n");
        return 1;
    }
...
//        vga_set_color(index, (index & 0x07) * 9, ((index >> 3) & 0x07) * 9, (index >> 6) * 21);
    ...
    while (active)
    {
     ...
     //   log_variable("mickeys_x", mickeys_x);
      //  log_variable("mickeys_y", mickeys_y);
      //  log_variable("rate_x", rate_x);
        

        // If both buttons are down, get ready to exit on release.
        if (bios.left && bios.right)
        {
          //  log_message("Both button pressed\n");
            left_clicked = false;
            exit_clicked = true;
        }
        ...
        {
        //    log_message("left button pressed\n");
            left_clicked = true; 
        }
...
void log_message(const char *message) {
    FILE *log_file = fopen("log_var.txt", "a");
    if (log_file) {
        fprintf(log_file, "%s\n", message);
        fclose(log_file);
    }
